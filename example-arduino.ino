...
int     sat     = 255;    // Currently-active animation mode, 0-9
int     val     = 60;    // Currently-active animation mode, 0-9
...
void rainbow(int wait) {
  ...
  for(long firstPixelHue = 0; firstPixelHue < 3*65536; firstPixelHue += 256) {
    for(int i=0; i<strip.numPixels(); i++) { // For each pixel in strip...
      ...
      strip.setPixelColor(i, strip.gamma32(strip.ColorHSV(pixelHue,sat,val)));
    }
    strip.show(); // Update strip with new contents
    delay(wait);  // Pause for a moment
  }
  ...
  for(int a=0; a<30; a++) {  // Repeat 30 times...
    for(int b=0; b<3; b++) { //  'b' counts from 0 to 2...
      ...
      for(int c=b; c<strip.numPixels(); c += 3) {
        ...
        uint32_t color = strip.gamma32(strip.ColorHSV(hue,sat,val)); // hue -> RGB
        strip.setPixelColor(c, color); // Set pixel 'c' to value 'color'
      }
      strip.show();                // Update strip with new contents
      delay(wait);                 // Pause for a moment
